                    ...
                    if (checkWritingOrNot == 1) {
                        ...
                        if (count == 0) {
                            ...
                            if(distance <= 100){
                                ...
                                // 거리 차이가 나게 되면, 카운트 다시 1로 바꾸고 새 좌표를 기준점으로 삼는다.
                                // (이건 시간 계산 끝난 후에 실행함)

                                // 이때, count 값이 작으면 아무 일도 일어나지 않지만,
                                // 만약 count가 일정 값 이상일 경우(한 곳에 오래 머물렀을 경우)
                                // 그 자리에 마커를 생성하는 동시에, 얼마나 오래 머물렀는지도 기록한다.
                                ...
                                // 일단 카운터 판별이 다 끝나면 count의 값을 바꿔준다.
                                count = 0
